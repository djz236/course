    ...
    <table id="simple-table" class="table  table-bordered table-hover">
      ...
      <td>
        <div class="hidden-sm hidden-xs btn-group">
          <button v-on:click="edit(courseContent)" class="btn btn-xs btn-info">
            <i class="ace-icon fa fa-pencil bigger-120"></i>
          </button>
          <button v-on:click="del(courseContent.id)" class="btn btn-xs btn-danger">
...
<script>
  ...
  export default {
    ...
    mounted: function() {
      ...
      _this.list(1);
      // sidebar激活样式方法一
      // this.$parent.activeSidebar("business-courseContent-sidebar");
    ...
    methods: {
      ...
      save() {
        ...
        // 保存校验
        if (1 != 1
          || !Validator.require(_this.courseContent.content, "课程内容")
        ) {
          return;
        }
      ...
      del(id) {
        let _this = this;
        Confirm.show("删除课程内容后不可恢复，确认删除？", function () {
          Loading.show();
